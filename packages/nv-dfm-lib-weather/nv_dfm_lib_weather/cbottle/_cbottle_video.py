...
# CBottle Video lives under px (prognostic)
from earth2studio.models.px.cbottle_video import CBottleVideo as CBottleVideoModel
...
class CbottleVideo:
    """
    CBottle Video model for prognostic climate forecasting.

    This class provides functionality to generate temporal forecasts (videos) of climate
    evolution using the CBottleVideo prognostic model. Starting from an initial climate
    state, the model generates future timesteps by iteratively rolling forward in time,
    producing physically consistent climate trajectories.

    CBottle Video is useful for:
    - Short to medium-range climate forecasting
    - Generating temporal sequences for visualization and animation
    - Exploring climate system evolution from given initial conditions
    - Creating ensemble forecasts with different random seeds
    - Producing training data for downstream models

    The model uses a diffusion-based prognostic approach, where each forecast step
    conditions on the previous state to generate the next timestep, maintaining
    temporal consistency throughout the trajectory.
    ...
    def __init__(self, site: Site, provider: Provider | None):
        """
        Initialize the CBottle Video prognostic model.
    ...
    async def valid_device(self, _value, _context):
        ...
        and informs clients about which devices can be used for running the CBottle Video model.
    ...
    async def body(
        ...
        """
        Generate prognostic climate forecasts using the CBottle Video model.

        This method performs iterative forecasting (time-stepping) from an initial climate state,
        generating future timesteps by repeatedly applying the CBottleVideo prognostic model.
        The model creates a temporal trajectory by conditioning each forecast step on the
        previous state, producing physically consistent climate evolution.

        The forecasting uses an autoregressive approach where:
        1. The first timestep from the input dataset serves as the initial condition
        2. The model generates the next timestep conditioned on the current state
        3. This process repeats for n_steps, creating a temporal sequence (video)

        Each forecast step internally uses diffusion-based generation to produce realistic
        climate fields that evolve naturally from the initial conditions.
        ...
        Args:
            dataset: Input xarray.Dataset containing the initial climate state. Must have dimensions:
                    - time: temporal dimension (at least one timestep)
                    - lat: latitude dimension
                    - lon: longitude dimension
                    The dataset must contain all variables required by the CBottle Video model.
            ...
            n_steps: Number of forecast steps to generate into the future. Each step typically
                    represents a fixed time interval (e.g., 6 hours, 1 day) depending on the
                    model's training. More steps produce longer forecasts but may accumulate
                    more uncertainty. Typical range: 4-24 steps. (default: 12)
        ...
        Returns:
            xarray.Dataset: Temporal forecast sequence.
        ...
        Example:
            >>> # Generate 24-hour forecast from initial conditions
            >>> initial_state = xr.Dataset({...})  # Contains initial climate state
            >>> result = await video.body(
            ...     dataset=initial_state,
            ...     n_steps=24,  # Generate 24 timesteps into the future
            ...     seed=42,
            ...     device="cuda:0"
            ... )
            >>> # Result contains initial state + 24 forecast timesteps
            >>> print(result.dims["time"])  # 25 (initial + 24 forecast)
        """
        cbottle_video_model = None
        try:
            # Validate inputs
            if dataset is None:
            ...
            if seed is not None:
                ...
                self._logger.info(f"Set CBottle Video seed to: {seed}")
            ...
            def _loader():
                package = CBottleVideoModel.load_default_package()
                return CBottleVideoModel.load_model(
                    package,
                    seed=seed,
                    lat_lon=lat_lon,
                )

            # Load the CBottleVideo model from cache or initialize it if not cached
            # The cache key includes lat_lon and seed to ensure proper model configuration
            cache = self._site.model_cache
            cache_key = f"cbottle.video:latlon={bool(lat_lon)}:seed={seed}"
            cbottle_video_model = cache.get_or_load(
                model_id=cache_key,
                loader=_loader,
                device=str(local_device),
            )
            self._logger.info(
                f"CBottleVideo model loaded on {local_device}; seed={seed}"
            )

            # Query the model for its required input variables and verify they exist in the dataset
            required_variables = list(cbottle_video_model.input_coords()["variable"])  # type: ignore[index]
            missing_vars = set(required_variables) - set(dataset.data_vars.keys())
            if missing_vars:
                raise ValueError(
                    "Dataset is missing variables required for CBottle Video: "
            ...
            x, coords = map_coords(x, coords, cbottle_video_model.input_coords())
            ...
            def run_unroll():
                ...
                iterator = cbottle_video_model.create_iterator(x, coords)
                ...
                if not outputs:
                    raise RuntimeError("CBottle Video iterator produced no outputs")
            ...
            if output_coords is None:
                raise RuntimeError(
                    "Missing output coordinates from CBottle Video iterator"
                )
            ...
            self._logger.info(
                f"CBottle Video unroll completed. Steps: {n_steps}. Output variables: {list(ds.data_vars.keys())}"
            )
            return ds
        except Exception as e:
            self._logger.error(f"Error performing CBottle Video unroll: {e}")
            raise
