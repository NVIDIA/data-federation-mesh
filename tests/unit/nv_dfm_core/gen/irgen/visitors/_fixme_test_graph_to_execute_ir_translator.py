...
from nv_dfm_core.gen.irgen.visitors import GraphToExecuteIRTranslator
...
def test_empty_graph_creates_minimal_net_ir(caplog):
    ...
    logger = Logger("test")
    translator = GraphToExecuteIRTranslator(logger=logger)
...
def test_graph_with_greetme_and_yield_creates_execution_net_ir(caplog):
    ...
    # Assert the transition body contains an AdapterCallStmt and SendStmt
    assert len(transition.body) == 2
    assert isinstance(transition.body[0], AdapterCallStmt)
    assert isinstance(transition.body[1], SendStmt)

    # Assert out places match the yield statement
    assert len(transition.out) == 1
    assert transition.out[0].to_place == yield_stmt.place
...
def test_graph_with_islands_and_params(caplog):
    ...
    greet1_set_field_node = graph.create_node(
        ordering=0.75,  # before the greet1 node
        location=("server",),
        node_model=SlotModel(slotname="name"),
    )
    greet1_set_field_node.state = NodeState.SELECTED

    greet2_set_field_node = graph.create_node(
        ordering=1.75,  # before the greet2 node
        location=("reception",),
        node_model=SlotModel(slotname="name"),
    )
    greet2_set_field_node.state = NodeState.SELECTED
    ...
    # cut nodes: edge went from greet1_node to greet2_set_field_node
    ...
    e = graph.create_edge(
        ...
        target=greet1_set_field_node,
        edge_model=FlowEdge(send_cost_info=None),
    )
    e.target.edges_used_to_compute_cost.append(e)
    e = graph.create_edge(
        source=greet1_set_field_node, target=greet1_node, edge_model=SlotEdge()
    )
    e.target.edges_used_to_compute_cost.append(e)
    e = graph.create_edge(
        source=greet1_node,
    ...
    e = graph.create_edge(
        ...
        target=greet2_set_field_node,
        edge_model=FlowEdge(send_cost_info=None),
    )
    e.target.edges_used_to_compute_cost.append(e)
    e = graph.create_edge(
        source=greet2_set_field_node, target=greet2_node, edge_model=SlotEdge()
    )
    e.target.edges_used_to_compute_cost.append(e)
    e = graph.create_edge(
        source=greet2_node,
    ...
    assert net_irs["server"] == NetIR(
        ...
        trans=[
            Transition(
                act=ActivationCondition(data_places=["myname"], opt=[]),
                body=[
                    ...
                    AdapterCallStmt(
                        ...
                        litparams={},
                        ssaparams={"name": StmtRef(ssa_def="myname")},
                        is_async=True,
                    ),
                    SendStmt(
                        job=None,
                        site="reception",
                        place="node1_name",
                        data=StmtRef(ssa_def="node0"),
                    ),
                ],
                out=[OutPlace(job=None, to_site="reception", to_place="node1_name")],
            )
        ],
    )
    ...
    assert net_irs["reception"] == NetIR(
        ...
        trans=[
            Transition(
                act=ActivationCondition(data_places=["node1_name"], opt=[]),
                body=[
                    ...
                    AdapterCallStmt(
                        ...
                        adapter="users_GreetMe",
                        litparams={},
                        ssaparams={"name": StmtRef(ssa_def="node1_name")},
                        is_async=True,
                    ),
                    SendStmt(
                        job=None,
                        site=None,
                        place="yield",
                        data=StmtRef(ssa_def="node1"),
                    ),
                ],
                out=[OutPlace(job=None, to_site=None, to_place="yield")],
            )
        ],
    )
