...
class TestPlaceParamsDelayed(PipelineCompilationTestBase):
    @staticmethod
    ...
    def cases() -> Generator[PipelineTestCase, Any, None]:
        for pipeline in TestPlaceParamsDelayed.pipelines():
            yield PipelineTestCase(
                test_name=None,
                test_variant=None,
                pipeline=pipeline,
                candidate_sites=["site1", "site2", "site3"],
                fed_info=TestPlaceParamsDelayed.fed_info(),
                expected_net_irs=None,
                run_params=[{"my_param": "first"}],
                delayed_params=[{"my_param": "second"}, {"my_param": "third"}],
                debug=False,
            )
    ...
    def test_compile_and_run_single_pipeline(
        ...
        assert len(result) == 10, (
            f"Expected 10 result tokens, got {str(len(result))}: {result}"
        )
        ...
        # second delayed params
        assert (
            TokenResponse(
                from_site="site3",
                from_node=1,
                frame=Frame(token="@dfm-control-token", frame=[2]),
                to_place="yield",
                data="unary('third')",
            )
            in result
        ), "Expected yield token from site3 for frame 2"
        assert (
            TokenResponse(
                from_site="site2",
                from_node=0,
            ...
            in result
        ), "Expected yield token from site2 for frame 2"

        # per-frame stop tokens
        assert (
            TokenResponse(
                from_site="site2",
                from_node=None,
        ...
        assert (
            ...
            in result
        ), "Expected stop token from site2 for frame 2"
